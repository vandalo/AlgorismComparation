...
public:
   ...
   : min_size(1),
     ...
     min_hashes(1),
     max_hashes(numeric_limits<unsigned int>::max()),
     expected_elements(10000),
     false_positive_p(1.0 / expected_elements),
...
public:
     ...
     desired_false_positive_p_(p.false_positive_p)
   {
	...
	inline bloom_filter& operator &= (const bloom_filter& f) {
		/* intersection */
		if ( (salt_count_  == f.salt_count_) &&
			(table_size_  == f.table_size_) && (random_seed_ == f.random_seed_)) {
